...
if (isset($_POST['email']) && isset($_POST['password'])) {
    ...
    $merchant_sql = $con->prepare("SELECT * FROM merchant WHERE email='$email'");
    $merchant_result = $merchant_sql->execute();
    $merchant = $merchant_sql->fetchAll();
    $merchant_count = $merchant_sql->rowCount();

    $shipping_company_sql = $con->prepare("SELECT * FROM shipping_company WHERE email='$email'");
    $shipping_company_result = $shipping_company_sql->execute();
        ...
        if ($user_count > 0) {
            foreach ($user as $item) {
                if ($item['email'] === $email) {
                    ...
                    $result = $con->exec($sql);
                    header("Location: login.php?message=success_msg");
                }
            }
            $con = null;
            ...
            foreach ($merchant as $item) {
                if ($item['email'] === $email) {
                    $sql = "UPDATE merchant SET password='$password' WHERE email='$email'";
                    $result = $con->exec($sql);
                    header("Location: login.php?message=success_msg");
                }
            }
            ...
            foreach ($shipping_company as $item) {
                if ($item['email'] === $email) {
                    $sql = "UPDATE shipping_company SET password='$password' WHERE email='$email'";
                    $result = $con->exec($sql);
                    header("Location: login.php?message=success_msg");
                }
            }
        ...
        <div class="w-full bg-white rounded-lg shadow  md:mt-0 sm:max-w-md xl:p-0  ">
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" method="post">
                    ...
                    <div class="flex flex-col items-end">
                        <label for="password" class="block mb-2 text-sm font-medium text-gray-900 ">الرقم السري
                            الجديد</label>
