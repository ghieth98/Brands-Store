<?php
session_start();
include "../connection.php";
$shipping_company = $_SESSION['shipping_company_id'];
$name = $_POST['name'];
$sql = "SELECT * FROM orders join brands.product p on p.product_id = orders.product_id WHERE  product_name LIKE '$name%' AND shipping_company_id='$shipping_company' AND order_status != 'قيد الانتظار'";
$query = $con->query($sql);

$data = '';
if ($query->rowCount() > 0) {
    while ($order = $query->fetch()) {
        $order_id = $order['order_id'];

        $data .= '<tr class="bg-white border-b hover:bg-gray-50 ">
                                <td class="w-4 p-4"></td>
                                <td class="text-center py-4"></td>

                                <td class="text-center py-4">
                                    <form method="post">
                                                <input type="hidden" value=" '.$order['product_id'] .'">
                                        <label for="order_status">
                                            <select name="order_status" id="order_status"
                                                    class="bg-green-500 text-white border border-viridian-green-500 rounded-md hover:bg-transparent hover:text-viridian-green-500 font-medium text-sm px-5 py-2.5 inline-flex items-center">
                                                <option selected disabled>تغيير الحالة</option>
                                                <option class="w-full text-sm font-medium text-gray-900 rounded"
                                                        value="تم الشحن">
                                                    تم الشحن
                                                </option>
                                                <option class="w-full text-sm font-medium text-gray-900 rounded"
                                                        value="تم التوصيل">
                                                    تم التوصيل
                                                </option>
                                            </select>
                                        </label>
                                    </form>
                                </td>

                                <td class="text-center py-4 font-bold">
                                <span class="px-2 py-1.5 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">
                                '. $order['order_status'] . ' 
                                </span>
                                </td>

                                <td class="text-center py-4 font-bold">
                                    SAR  '.$order['price'] . ' 
                                </td>

                                <td class="text-center py-4 font-bold">
                                 '.    substr($order['description'], 0, 100) . ' 
                                </td>
                                <td class="text-center py-4">
                                    <div class="flex flex-col items-center">
                                        <img src="../assets/uploads/'.$order['image'].' " alt="product image"
                                             class=" w-20 rounded">
                                    </div>
                                </td>
                                <td class="text-center py-4 font-bold">
                                   '.  $order['product_name'] . ' 
                                </td>
                            </tr>';


    }

} else {
    $data .= '<tr class="bg-white border-b hover:bg-gray-50 ">
                                <td class="w-4 p-4"></td>
                                <td class="text-center py-4"></td>
                                <td class="text-center py-4">
                                </td>
                                <td class="text-center py-4 font-bold">
                                </td>
                                <td class="text-center py-4 font-bold">
                                </td>
                                <td class="text-center  py-4 font-bold">
                                </td>
                                <td class="text-center text-lg font-bold py-4">
                                         لا يوجد
                                </td>
                                <td class="text-center py-4 font-bold">
                                </td>
                            </tr>';
}

echo $data;

