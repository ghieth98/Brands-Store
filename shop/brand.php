...
$query = $con->prepare("SELECT * FROM merchant WHERE merchant_id='$merchant_id'");
$query->execute();
$merchant = $query->fetch();

$products = $con->query("SELECT * FROM product JOIN merchant ON product.merchant_id = merchant.merchant_id WHERE product.merchant_id='$merchant_id' AND product.status='عام'");
...
<div class="px-8 mt-12 grid md:grid-cols-3 grid-cols-2 gap-6 pt-4 pb-16 items-start">
    ...
    <div class="col-span-3">
        <div class="flex flex-col border-b pb-2  items-end mr-5">
            <div class="px-8 pt-4 grid grid-cols-2 gap-6">

                <div class="flex flex-col mt-12 pt-8 items-end text-right">
                    <h2 class="text-4xl font-medium uppercase mb-2">
                        <?php
                        echo $merchant['name']
                        ?>
                    </h2>
                    ...
                    <div class="space-y-2">
                        <p class="text-gray-800 text-xl font-semibold space-x-2">
                            <span> رقم الهاتف</span>
                            <span class="text-green-600">: <?php echo $merchant['phone'] ?></span>
                        </p>

                        <p class="space-x-2 text-xl">
                            <a href="<?php echo $merchant['commercial_register_link'] ?>" target="_blank"
                               class=" hover:underline">
                                <?php echo $merchant['commercial_register'] ?>
                            </a>
                            <span class="text-gray-800 hover:text-green-500 font-semibold">:
                            <span class="text-gray-600">سجل تجاري</span>
                        </p>
                    </div>
                </div>

                <div class="flex flex-col text-right">
                    <img src="../assets/uploads/<?php echo $merchant['logo'] ?>" alt="product"
            ...
            <h1 class="mb-8 mt-8 text-3xl font-semibold text-gray-700 dark:text-gray-200">
                منتجات البائع

            </h1>
        </div>
        <div dir="rtl" class="grid pt-8 grid-cols-1 md:grid-cols-4 gap-6">
            <?php
