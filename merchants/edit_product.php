...
$query = $con->prepare("SELECT * FROM product WHERE product_id='$product_id'");
$query->execute();
$product = $query->fetch();
?>
        ...
        if (isset($_POST['submit'])) {
            ...
            if (isset($_FILES['image'])) {
                ...
                if (!empty($new_image_name)) {
                    ...
                    $upload_path = '../assets/uploads/' . $new_image_name;
                    move_uploaded_file($_FILES['image']['tmp_name'], $upload_path);
                } else {
                    $new_image_name = $old_image_name;
                }
                //Sql insert
                $sql = "UPDATE product SET product_name='$product_name', price='$price', description='$description', image='$new_image_name', status='$status', quantity='$quantity', category_id='$category' WHERE product_id='$product_id'";
                $result = $con->exec($sql);
                    ...
                    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                        <div class="flex flex-col items-end pl-4">
                            <h2 class="mb-4 text-xl font-bold text-gray-900 ">
                                عدل <?php echo $product['product_name'] ?></h2>
                        ...
                        <form class="space-y-4 md:space-y-6 " method="post" enctype="multipart/form-data">
                            ...
                            <div class="flex flex-col items-end">
                                <label for="status"
                                       class="block mb-2 text-sm font-medium text-gray-900 ">الحالة</label>
                                <select dir="rtl" type="text" name="status" id="status"
                                        class="bg-gray-50 text-right border border-gray-300 sm:text-sm rounded-lg focus:ring-green-600 focus:border-green-600 block w-full p-2.5">
                                    <option value="عام">عام</option>
                                    <option value="مختفي">مختفي</option>
                                </select>
                            </div>
                            <div class="flex flex-col items-end">
                                <label for="quantity"
